...
Snow::Snow() {
    ...
    mUpdateShader = new Shader("../src/shaders/particle/Update.vs", "../src/shaders/particle/Update.fs",
                               "../src/shaders/particle/Update.gs", varyings, 5);
    //设置TransformFeedback缓存能够记录的顶点的数据类型

    mRenderShader = new Shader("../src/shaders/particle/Render.vs",
                               "../src/shaders/particle/Render.fs");
    //设置随机纹理
...
void Snow::UpdateParticles(float frametimeMills) {
    ...
    glVertexAttribPointer(3, 1, GL_FLOAT, GL_FALSE, sizeof(SnowParticle),
                          (void *) offsetof(SnowParticle, lifetimeMills));
