...
void AnimationController::InitController() {
	AnimationModel* model = new AnimationModel("../resources/animation/Character Running.fbx", 
												glm::vec3(0.005f), glm::vec3(5.0f, 0.8f, 5.0f), glm::vec3(0.0f, 0.0f, 0.0f));

	animation_models.push_back(model);

	// Init shader
	this->depth_shader = Shader("../src/shaders/animation/shadow_mapping_depth_dynamic.vs", "../src/shaders/shadow_frag.glsl");
