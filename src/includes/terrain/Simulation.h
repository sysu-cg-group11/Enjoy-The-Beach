...
#include <terrain/framebuffer.h>
#include <terrain/grid.h>
#include <terrain/perlinTex.h>
...
class Simulation {
  ...
  private:
    void cameraMovements(float phi, float coef) {
        ...
        cam_phi = glm::clamp(cam_phi + cam_speed_phi * coef, CAM_PHI_MIN, CAM_PHI_MAX);
    }
    ...
    void set_noise_params(int doctave, float dlacunarity, float dfractal_increment) {
        ...
        octave = glm::clamp(octave, 1, 9);
        lacunarity = glm::clamp(lacunarity, 0.0f, 10.0f);
        fractal_increment = glm::clamp(fractal_increment, 1.0f, 2.0f);
  ...
  public:
    void onMouseMove(GLFWwindow *window, double x, double y) {
        ...
        if (camera_mode != B_PATH) {
            ...
            cam_phi = glm::clamp(cam_phi, CAM_PHI_MIN, CAM_PHI_MAX);
            cursor_x = x;
            cursor_y = y;
        }
    }
